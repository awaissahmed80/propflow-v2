...
function InputGroup({
  ...
  return (
    <div
      ...
      role="group"
      className={cn(
        "group/input-group relative flex h-control w-full min-w-0 items-center rounded-md border border-input bg-transparent shadow-xs transition-[color,box-shadow] outline-none",
        "dark:bg-input/30",
  ...
  "flex h-auto cursor-text items-center justify-center gap-2 py-1.5 text-sm font-medium text-muted-foreground select-none group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-[calc(var(--radius)-5px)] [&>svg:not([class*='size-'])]:size-4",
  {
    variants: {
      align: {
        ...
        "inline-end":
          "order-last pr-0.5 has-[>button]:-mr-1 has-[>kbd]:mr-[-0.15rem]",
        "block-start":
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (
          (e.target).closest(
            "button,[data-slot=select-trigger],[data-slot=combobox-trigger],[data-slot=input-group-control],[role=combobox]"
          )
        ) {
          return
        }
...
/**
 * Borderless NumberInput for use inside InputGroup.
 * Steppers are off by default so the group stays clean.
 */
