...
function Avatar({
  ...
  return (
    <RXAvatar size={size} className={className} {...rest}>
      {src ? <AvatarImage src={src} alt={alt || name} /> : null}
      <AvatarFallback
        style={{ backgroundColor: stringToColor(name) }}
        className={cn(
          "text-xs font-semibold text-white select-none group-data-[size=sm]/avatar:text-[10px] group-data-[size=lg]/avatar:text-sm",
          textClass
        )}
      >
        {getInitials(name)}
